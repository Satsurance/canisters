...
fn test_multiple_users_different_deposits_proportional_rewards() {
    ...
    reward_pool(&pic, canister_id, ledger_id, user_a, reward_amount.clone()).expect("Reward pool should succeed");
    
    // Advance to middle of reward period
    let exact_reward_duration = ((reward_start_time + EPISODE_DURATION * 12) / EPISODE_DURATION + 1) * EPISODE_DURATION - reward_start_time;
    advance_time(&pic, exact_reward_duration / 2);
    
    let rewards_a = pic.query_call(canister_id, user_a, "get_deposits_rewards", encode_args((vec![0u64],)).unwrap())
        .map(|r| decode_one::<Nat>(&r).unwrap()).expect("Failed to get user A rewards");
    
    let rewards_b = pic.query_call(canister_id, user_b, "get_deposits_rewards", encode_args((vec![1u64],)).unwrap())
        .map(|r| decode_one::<Nat>(&r).unwrap()).expect("Failed to get user B rewards");
    
    // Get actual shares and calculate expected rewards
    let deposits_a: Vec<icp_canister_backend::UserDepositInfo> = decode_one(&pic
        .query_call(canister_id, user_a, "get_user_deposits", encode_args((user_a,)).unwrap()).unwrap()).unwrap();
    let deposits_b: Vec<icp_canister_backend::UserDepositInfo> = decode_one(&pic
        .query_call(canister_id, user_b, "get_user_deposits", encode_args((user_b,)).unwrap()).unwrap()).unwrap();
    ...
    assert_with_error!(&rewards_a, &expected_a, &ALLOWED_ERROR, "User A proportional rewards");
    assert_with_error!(&rewards_b, &expected_b, &ALLOWED_ERROR, "User B proportional rewards");
}
...
fn test_reward_withdrawal_ownership_and_security() {
    ...
    let result: Result<Nat, icp_canister_backend::PoolError> = decode_one(&malicious_result).unwrap();
    let is_not_owner_error = matches!(result, Err(icp_canister_backend::PoolError::NotOwner));
    assert!(is_not_owner_error, "Expected NotOwner error, got: {:?}", result);
    ...
    let malicious_result_2 = malicious_withdrawal_2
        .expect("Malicious withdrawal call should not fail at protocol level");
    let result_2: Result<Nat, icp_canister_backend::PoolError> = decode_one(&malicious_result_2).unwrap();
    let is_not_owner_error_2 = matches!(result_2, Err(icp_canister_backend::PoolError::NotOwner));
    assert!(is_not_owner_error_2, "Expected NotOwner error for malicious user, got: {:?}", result_2);
...
fn test_multiple_reward_pool_additions_cumulative() {
    ...
    let first_end = ((first_reward_time + EPISODE_DURATION * 12) / EPISODE_DURATION + 1) * EPISODE_DURATION;
    let second_end = ((second_reward_time + EPISODE_DURATION * 12) / EPISODE_DURATION + 1) * EPISODE_DURATION;
    let third_end = ((third_reward_time + EPISODE_DURATION * 12) / EPISODE_DURATION + 1) * EPISODE_DURATION;
    ...
    assert_with_error!(&final_rewards, &total_expected_rewards, &ALLOWED_ERROR, "Multiple reward additions final total");
}
#[test]
fn test_reward_distribution_between_large_and_small_deposits() {
    ...
    let small_deposit = icp_canister_backend::MINIMUM_DEPOSIT_AMOUNT.clone() + Nat::from(1u64);
    let reward_amount = Nat::from(1_000u64); // $1

    let episode = get_stakable_episode(&pic, canister_id, 7);

    create_deposit(&pic, canister_id, ledger_id, large_depositor, large_deposit, episode);
    create_deposit(&pic, canister_id, ledger_id, small_depositor, small_deposit, episode);

    let reward_start_time = pic.get_time().as_nanos_since_unix_epoch() / 1_000_000_000;
    reward_pool(&pic, canister_id, ledger_id, large_depositor, reward_amount.clone())
        .expect("Reward pool should succeed");
    ...
    let large_depositor_rewards = pic
        .query_call(canister_id, large_depositor, "get_deposits_rewards", encode_args((vec![0u64],)).unwrap())
        .map(|r| decode_one::<Nat>(&r).unwrap())
        .expect("Failed to get large depositor rewards");
    
    let small_depositor_rewards = pic
        .query_call(canister_id, small_depositor, "get_deposits_rewards", encode_args((vec![1u64],)).unwrap())
        .map(|r| decode_one::<Nat>(&r).unwrap())
        .expect("Failed to get small depositor rewards");

    let total_distributed = large_depositor_rewards + small_depositor_rewards;
    assert_with_error!(&total_distributed, &reward_amount, &ALLOWED_ERROR, "Total distributed equals reward");
}